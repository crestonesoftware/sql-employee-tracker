...
async function updateEmployeeRole() {
  const answers = await inquirer.prompt(qu.updateEmployeeRole);
  const { role, employeeID } = answers;
  qu.fetchEmployeeNames(pool);
  qu.fetchRoleNames(pool);
...
async function addRole() {
  const answers = await inquirer.prompt(qu.addRole);
